...
- `GET /discovery/recipes` — Filtered recipe discovery
  - Query params: `genreId`, `varietyId`, `excludeAllergens` (comma-separated allergen IDs — excludes recipes that either tag the allergen manually via `recipes.allergen_ids` OR contain it through any ingredient via `ingredient_allergens`. Derived exclusion is required because `allergen_ids` is a denormalized column most creators leave empty; see #518), `tagIds` (comma-separated dietary tag IDs — only recipes with ALL specified tags), `culturalTagIds` (comma-separated cultural tag IDs — recipes matching ANY of them; OR logic), `search` (Turkish-aware partial match on recipe title; runs the ilike-variants OR against `recipes.title` **and** against `recipe_translations.title`, so a query in either EN or TR matches the recipe regardless of authored language — e.g. `"English"` finds a TR-authored recipe via its EN translation row, and `"İngilizce"` finds an EN-authored recipe via its TR translation row), `country`, `city`, `district` (case-insensitive, whitespace-/diacritic-tolerant; country also resolves common aliases — `"tr"`/`"Türkiye"`/`"TUR"` all match recipes stored as `"Turkey"`. See Location Normalization below), `page`, `limit`
